...
// Message Sending
    ...
    ()=> slackBot.methods.chat.postMessage({ channel: 'general', text: 'This is a regular, text-based message.' }),

    // Regular message, with as_user, with text
    ()=> slackBot.methods.chat.postMessage({ channel: 'general', text: 'This is a message with as_user set to true, displaying parameter behavior.', as_user: true }),
...
// Storage
    ...
    ()=>{
        for(let slackId of Object.keys(slackBot.cache.users)){
            const user = slackBot.cache.users[slackId];
            if(user.name == 'jake861'){
                log.info('');
        ...
        log.info('');
        log.info(`Could not find the user ${log.chalk.cyan('jake861')}.`);
        log.info('')
    }
...
function doTest() {
    if(tests.length){
        ...
        if(timeToDelay == undefined || !timeToDelay){
